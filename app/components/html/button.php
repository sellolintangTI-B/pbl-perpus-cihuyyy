...
switch ($color) {
    ...
    case 'red':
        ...
        break;
    case 'white':
        $customClass = "bg-white text-black/80 rounded-xl flex gap-2 items-center justify-center border-2 shadow-sm border-white hover:border-black/80 hover:shadow-lg transition-all  duration-500";
        break;
}

?>
<button class=" <?= $customClass ?> <?= $class ?> shadow-md shadow-black/25 font-medium cursor-pointer" type="<?= $type ?>" onclick="<?= $onClick ?>" id="<?= $id ?>" name="<?= $name ?>" @click="<?= $alpineClick ?>">
