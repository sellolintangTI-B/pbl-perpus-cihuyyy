...
{
    public static function getUserIdByBookingId($id)
    {
        $conn = parent::getConnection();
        $q = $conn->prepare("SELECT user_id FROM booking_participants WHERE booking_id = ?");
        $q->bindValue(1, $id);
        $q->execute();
    ...
    public static function deleteParticipantsWhereInIds($id, $uuids = [])
    {
        $conn = parent::getConnection();
        $placeholder = implode(',', array_fill(0, count($uuids), '?'));
        $sql = "DELETE FROM booking_participants WHERE booking_id = '$id' AND user_id IN ($placeholder)";
        $q = $conn->prepare($sql);
        $q->execute($uuids);
        if($q) return true;
        return false;
